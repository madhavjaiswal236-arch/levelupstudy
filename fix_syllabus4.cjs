...
code = code.replace(/const allTasksForChapter = \[\.\.\.todos[^]*?\}\)\)\)\)\)\.length; const dppTodos = chapterTodos\.filter\(t => t\.type === 'DPP' \|\| \(t\.text && t\.text\.toLowerCase\(\)\.includes\('dpp'\)\)\);/, replacement.replace(/\n/g, ' '));
