import re

with open('src/components/StudyCalendar.tsx', 'r') as f:
    content = f.read()

# 1. Update Supreme Line
supreme_line = r'''{/* Supreme Line (Current Time) */}
          <div 
            className="absolute w-full h-[2px] bg-indigo-600 z-[45] pointer-events-none shadow-[0_0_12px_rgba(79,70,229,0.8)]" 
            style={{ top: `${(currentLocalTime.getHours() + currentLocalTime.getMinutes() / 60) * 80}px` }}
          >
            <div className="w-2.5 h-2.5 rounded-full bg-indigo-500 absolute -left-[5px] -top-[4px] shadow-[0_0_10px_rgba(79,70,229,1)]" />
          </div>'''

content = re.sub(
    r'\{\/\* Supreme Line.*?<\/div>',
    supreme_line,
    content,
    flags=re.DOTALL
)

# 2. Add Supreme Time Capsule in Time Column
time_col_replacement = r'''      <div className="w-16 flex-shrink-0 border-r border-slate-200 dark:border-white/5 relative z-20 bg-white dark:bg-[#121212]">
        <div className="h-12 border-b border-slate-200 dark:border-white/5 text-[10px] text-slate-400 flex justify-center items-end pb-2 sticky top-0 bg-white dark:bg-[#121212] z-[60]">
          TIME
        </div>
        {hours.map(hour => (
          <div key={hour} className="h-20 text-xs text-slate-500 flex justify-center pr-2 pt-2 relative border-b border-slate-100 dark:border-white/5 border-dashed">
            <span className="relative -top-2.5 bg-white dark:bg-[#121212] px-1">
              {hour === 12 ? '12 PM' : hour > 12 ? `${hour-12} PM` : hour === 0 ? '12 AM' : `${hour} AM`}
            </span>
          </div>
        ))}
        {/* Supreme Time Capsule */}
        <div 
          className="absolute left-0 w-full flex justify-center items-center z-[50] pointer-events-none"
          style={{ top: `${48 + (currentLocalTime.getHours() + currentLocalTime.getMinutes() / 60) * 80}px`, transform: 'translateY(-50%)' }}
        >
          <div className="bg-indigo-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-[0_0_12px_rgba(79,70,229,0.8)] border border-indigo-400/30">
            {format(currentLocalTime, 'h:mm a')}
          </div>
        </div>
      </div>'''

content = re.sub(
    r'<div className="w-16 flex-shrink-0 border-r border-slate-200 dark:border-white\/5 relative z-20 bg-white dark:bg-\[#121212\]">.*?<\/div>\s*<\/div>',
    time_col_replacement,
    content,
    flags=re.DOTALL
)

# 3. Update Day Headers
day_headers = r'''{/* Day Headers */}
        <div className="flex h-12 border-b border-slate-200 dark:border-white/5 sticky top-0 z-[60] bg-white/95 dark:bg-[#121212]/95 backdrop-blur-md shadow-[0_4px_16px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.4)]">
          {visibleDays.map(d => (
            <div key={d.getTime()} className="flex-1 flex items-center justify-center border-r border-slate-200 dark:border-white/5 relative gap-1.5">
              <span className={`text-[10px] font-bold uppercase tracking-wider ${isSameDay(d, new Date()) ? 'text-indigo-500' : 'text-slate-400'}`}>{format(d, 'EEE')}</span>
              <span className={`text-sm font-bold w-7 h-7 flex items-center justify-center rounded-full transition-all ${isSameDay(d, new Date()) ? 'bg-indigo-600 text-white shadow-[0_0_12px_rgba(79,70,229,0.8)]' : 'text-slate-700 dark:text-slate-200'}`}>
                {format(d, 'd')}
              </span>
            </div>
          ))}
        </div>'''

content = re.sub(
    r'\{\/\* Day Headers \*\/.*?<\/div>\s*<\/div>',
    day_headers + '\n        ',
    content,
    flags=re.DOTALL
)

with open('src/components/StudyCalendar.tsx', 'w') as f:
    f.write(content)

print("Patched timeline")
