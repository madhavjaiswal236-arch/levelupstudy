...
time_col_replacement = r'''      <div className="w-16 flex-shrink-0 border-r border-slate-200 dark:border-white/5 relative z-20 bg-white dark:bg-[#121212]">
        <div className="h-12 border-b border-slate-200 dark:border-white/5 text-[10px] text-slate-400 flex justify-center items-end pb-2 sticky top-0 bg-white/95 dark:bg-[#121212]/95 backdrop-blur-md z-[60]">
          TIME
        </div>
        {hours.map(hour => (
...
# 3. Update Day Headers for Capsule and Glowing Effect
...
# BE CAREFUL with regex replacement to avoid capturing the entire render function!
# We find '{/* Day Headers */}' and the next '</div>' that is followed by '{/* Grid Body */}'.
content = re.sub(
    r'\{\/\* Day Headers \*\/.*?(?=\{\/\* Grid Body \*\/)',
    day_headers + '\n        \n        ',
    content,
    flags=re.DOTALL
)
...
print("Patched supreme successfully")
