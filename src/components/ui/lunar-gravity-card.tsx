...
const ParticleRing = ({ ringState, massiveAsteroidsRef }: { ringState: 'hidden' | 'animating' | 'visible', massiveAsteroidsRef: React.MutableRefObject<Float32Array> }) => {
  ...
  // Persistent matrix, vector and buffer to eliminate GC allocations per frame
  const invMatRef = useRef(new THREE.Matrix4());
  const astVecRef = useRef(new THREE.Vector3());
  const localAsteroidsRef = useRef(new Float32Array(75 * 4));
  ...
  useFrame((state, delta) => {
    if (pointsRef.current) {
      ...
      const localAsteroids = localAsteroidsRef.current;
      const astVec = astVecRef.current;
      ...
      for(let i=0; i<75; i++) {
        ...
        localAsteroids[i*4] = astVec.x;
        localAsteroids[i*4+1] = astVec.y;
        localAsteroids[i*4+2] = astVec.z;
...
export function LunarGravityCard({ 
  ...
  return (
    <div className={cn("w-full max-w-[1000px] min-h-[500px] bg-black/40 rounded-[2.5rem] flex flex-col md:flex-row relative overflow-hidden border border-white/[0.08] shadow-[0_30px_100px_rgba(0,0,0,0.4)] backdrop-blur-md", className)}>
      ...
      <div className="w-full md:w-[45%] flex flex-col justify-center px-10 py-12 md:p-0 md:pl-16 relative z-20 pointer-events-none">
        ...
        <p className="text-sm md:text-base text-zinc-400 font-medium leading-relaxed max-w-[340px]">
          {description}
        </p>
      </div>
...
export function LunarGravityCanvas({ onMoonClick }: { onMoonClick?: () => void }) {
  const [ringState, setRingState] = useState<'hidden' | 'animating' | 'visible'>('hidden');
  const massiveAsteroidsRef = useRef<Float32Array>(new Float32Array(75 * 4));
