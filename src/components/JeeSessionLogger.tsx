...
type SessionType =
  "Lecture" | "Theory" | "Practice" | "PYQs" | "Revision" | "DPP";
...
export default function JeeSessionLogger({
  pendingSessionLog,
  clearPendingSessionLog,
}: JeeSessionLoggerProps) {
  const {
    syllabus,
    addXp,
  ...
  const [dppStatus, setDppStatus] = useState<
    "solved" | "not_solved" | "not_provided" | "scheduled"
  >("solved");
  ...
  const calculatedDppAccuracy =
    parsedTotal > 0 ? Math.round((parsedScored / parsedTotal) * 100) : 0;
  ...
  useEffect(() => {
    if (
      ...
      if (sessionType === "Lecture" || sessionType === "Theory") {
        setLectureNumber(
          predictNextLecture(subject, chapter, todos, history, syllabus),
        );
    ...
    if (pendingSessionLog) {
      ...
      if (matchedTodo) {
        if (matchedTodo.durationMinutes) {
          ...
          const diffMs =
            new Date(matchedTodo.endTime).getTime() -
            new Date(matchedTodo.startTime).getTime();
  ...
  const handleLogSession = () => {
    ...
    let numHours =
      sessionType === "DPP" ? 1 : Math.max(0, parseFloat(hours as string) || 0);
    ...
    let updates: any = {
      ...
      status:
        newAccuracy >= 80 ? "green" : newAccuracy >= 50 ? "yellow" : "red",
      confidence:
        newAccuracy >= 80 ? "Strong" : newAccuracy >= 50 ? "Average" : "Weak",
    };

    if (sessionType === "Theory" || sessionType === "Lecture") {
      updates.lectures = isLectureComplete
        ? 100
        : Math.min(99, currentLectures + numHours * 10);
      const parsedLecNum = parseInt(lectureNumber as string);
      if (!isNaN(parsedLecNum) && parsedLecNum > 0) {
        const currentLastLec = currentStats?.lastLectureNumber || 0;
    ...
    const isOneShot =
      (sessionType === "Lecture" || sessionType === "Theory") && numHours > 4;

    // Save to daily logged entry
    const taskDetails =
      sessionType === "DPP"
        ? extraInfo.replace(" | ", "")
        : `${lectureNumber ? `Lec ${lectureNumber} ` : ""}(${numHours}h${isOneShot ? " One-Shot" : ""}${numQuestions > 0 ? `, ${numQuestions} Qs` : ""})${extraInfo}`;
    const newLoggedTask = {
      id: Date.now(),
      ...
      dppDone:
        ...
        sessionType === "Theory" ||
        sessionType === "DPP"
          ? dppStatus === "solved" || sessionType === "DPP"
          : undefined,
    };
    ...
    if (!pendingTaskId) {
      ...
      const isDuplicate = loggedTasksToday.some((t) => {
        ...
        if (
          ...
          if (
            newLoggedTask.lectureNumber !== undefined &&
            t.lectureNumber === newLoggedTask.lectureNumber
          )
            return true;
        ...
        >
          <div className="flex items-center gap-4 group/logger">
            <motion.div
              ...
              className="p-3 bg-cyan-500/20 rounded-xl border border-cyan-500/30 shadow-md"
            >
              <Activity className="w-6 h-6 dark:text-cyan-400 text-cyan-700 group-hover/logger:drop-shadow-md transition-all" />
            </motion.div>
            <div>
              <h2 className="text-2xl font-heading font-black dark:text-white text-slate-900 tracking-widest uppercase drop-shadow-md">
                Log JEE Session
              </h2>
              <p className="dark:text-cyan-400/70 text-cyan-700/70 text-xs font-bold tracking-widest uppercase mt-1">
                Report output to update your AIR trajectory
              </p>
            </div>
          </div>
          <ChevronDown
            className={`w-6 h-6 dark:text-cyan-400 text-cyan-700 transition-transform duration-300 ${isExpanded ? "rotate-180" : ""}`}
          />
        </div>
            ...
            >
              <CardContent className="p-6 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="text-xs font-bold dark:text-slate-400 text-slate-600 uppercase tracking-wider mb-3 block">
                      Subject
                    </label>
                      ...
                      {chapter ? (
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                            <span className="text-lg font-heading font-bold dark:text-white text-slate-900">
                              {chapter}
                            </span>
                            {chapter ===
                              getCurrentChapterForSubject(subject) && (
                        ...
                        <span className="dark:text-slate-500 text-slate-600 italic">
                          Click to select a chapter...
                        </span>
                      )}
                      <Button
                        variant="ghost"
                        size="sm"
                        className="dark:text-cyan-400 text-cyan-700 hover:dark:text-cyan-300 dark:text-cyan-400 text-cyan-700 hover:bg-cyan-500/10"
                      >
                  ...
                  {sessionType !== "DPP" && (
                    <div>
                      ...
                      {(() => {
                        ...
                        return (
                          <p className="text-[11px] dark:text-slate-500 text-slate-600 mt-2 tracking-wide whitespace-nowrap">
                            {hours} means {hrsStr} {minsStr}
                          </p>
                        );
                      })()}
                    </div>
                ...
                {sessionType !== "DPP" && (
                  <div>
                    <div className="flex justify-between mb-3">
                      <label className="text-xs font-bold dark:text-slate-400 text-slate-600 uppercase tracking-wider">
                        Accuracy: {accuracy}%
                      </label>
                      ...
                      {isLectureComplete ? (
                        <div className="w-6 h-6 flex items-center justify-center bg-cyan-500 rounded border border-cyan-400 shadow-md">
                          <svg
                            ...
                            className="w-4 h-4 dark:text-white text-slate-900"
                          >
                ...
                <Button
                  ...
                  disabled={
                    !subject ||
                    !chapter ||
                    (sessionType !== "DPP" && !hours) ||
                    (sessionType === "DPP" &&
                      (dppMarksScored === "" ||
                        dppTotalMarks === "" ||
                        parsedScored > parsedTotal ||
                        parsedTotal <= 0))
                  }
        ...
        <AnimatePresence>
          {showDppModal && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 dark:bg-black bg-slate-50 overflow-y-auto custom-scrollbar">
              ...
              >
                <h2 className="text-2xl font-black dark:text-white text-slate-900 mb-6 uppercase tracking-widest text-center">
                ...
                {sessionType !== "DPP" && (
                  <div className="space-y-4 mb-8">
                    ...
                    <Button
                      variant={
                        dppStatus === "not_solved" ? "destructive" : "outline"
                      }
                    ...
                    >
                      <motion.div whileHover={{ scale: 1.2, rotate: -15 }}>
                        <X className="w-4 h-4 mr-2 group-hover/dpp1:drop-shadow-md transition-all" />
                      </motion.div>{" "}
                      Did Not Solve DPP
                    </Button>
                    <Button
                      variant={
                        dppStatus === "not_provided" ? "secondary" : "outline"
                      }
                    ...
                    >
                      <motion.div whileHover={{ scale: 1.2, rotate: 15 }}>
                        <Activity className="w-4 h-4 mr-2 group-hover/dpp2:drop-shadow-md transition-all" />
                      </motion.div>{" "}
                      DPP Not Provided For Lecture
                    </Button>
                    <Button
                      variant={
                        dppStatus === "scheduled" ? "default" : "outline"
                      }
                    ...
                    >
                      <BookOpen className="w-4 h-4 mr-2 dark:text-amber-400 text-amber-700" />{" "}
                      Schedule for Later
                    </Button>
                  </div>
                )}
                ...
                <AnimatePresence>
                  {dppStatus === "not_solved" && (
                    ...
                    >
                      ⚠️ Warning: Since you skipped the DPP, you will receive a
                      50% XP penalty for this lecture session. The lecture
                      cannot be fully completed without practicing!
                    </motion.div>
                  )}
                  {dppStatus === "scheduled" && (
                    <motion.div
                    ...
                    >
                      ⚠️ Note: Since you scheduled the DPP for later, you will
                      receive a 25% XP penalty for this lecture session. Make
                      sure to complete it!
                    </motion.div>
                  )}
                  {dppStatus === "solved" && (
                    ...
                    >
                      <div className="flex justify-between mb-3">
                        ...
                        <span
                          className={`text-xs font-bold uppercase tracking-wider ${calculatedDppAccuracy >= 80 ? "dark:text-green-400 text-green-700" : calculatedDppAccuracy >= 50 ? "dark:text-yellow-400 text-yellow-700" : "dark:text-red-400 text-red-700"}`}
                        >
                          {parsedTotal > 0
                            ? `${calculatedDppAccuracy}% Accuracy`
                            : "Awaiting Input"}
                        </span>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="relative">
                          <label className="text-[10px] font-bold dark:text-slate-500 text-slate-600 uppercase absolute -top-2 left-3 dark:bg-slate-900 bg-white px-1">
                            Marks Scored
                          </label>
                          <input
                            type="number"
            ...
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 dark:bg-black bg-slate-50 ">
              ...
              >
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h3 className="text-2xl font-heading font-bold dark:text-white text-slate-900 uppercase tracking-wider">
                      Select Chapter
                    </h3>
                    <p className="dark:text-cyan-400 text-cyan-700 text-sm font-medium">
                      {subject}
                    </p>
                  </div>
                  <Button
                    ...
                    className="dark:text-slate-400 text-slate-600 hover:text-white hover:dark:bg-slate-800 bg-slate-100 rounded-full group/close"
                  >
                    <motion.div whileHover={{ scale: 1.2, rotate: 90 }}>
                      <X className="w-6 h-6 group-hover/close:drop-shadow-md transition-all" />
                    </motion.div>
                  </Button>
                </div>
                ...
                <div className="overflow-y-auto custom-scrollbar pr-2 flex-1">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {syllabus[subject].map((c) => {
                      const isRecommended =
                        getCurrentChapterForSubject(subject) === c.name;
                      return (
                        <button
                          key={c.name}
                          onClick={() => {
                        ...
                        >
                          <div className="flex items-center justify-between w-full">
                            <span
                              className={`text-sm font-bold leading-tight ${chapter === c.name ? "dark:text-cyan-400 text-cyan-700" : isRecommended ? "dark:text-emerald-400 text-emerald-700 font-bold" : "dark:text-slate-200 text-slate-900"}`}
                            >
                              {c.name}
                            </span>
                            {isRecommended && (
                          ...
                          <div className="flex gap-2 mt-auto pt-2">
                            <span className="text-[10px] px-2 py-1 rounded dark:bg-slate-800 bg-slate-100 dark:text-slate-400 text-slate-600 font-mono">
                              Lec: {c.lectures}%
                            </span>
                            <span className="text-[10px] px-2 py-1 rounded dark:bg-slate-800 bg-slate-100 dark:text-slate-400 text-slate-600 font-mono">
                              PYQ: {c.pyq}%
