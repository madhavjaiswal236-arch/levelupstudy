...
function AppContent() {
 ...
 const now = new Date();
 // Only rollover if current hour is past 3 AM
 
        const currentLogicalDate = getLogicalDate();

 const last = new Date(lastStudyDate); // e.g. previous day
 if (
 ...
 setNeedsRollover(true);
  ...
  }, isPowerSaver ? 300000 : 60000); // Check every minute

 return () => clearInterval(interval);
 }, [isLoaded, lastStudyDate, needsRollover, setNeedsRollover]);
 ...
 const [missedDayInputs, setMissedDayInputs] = useState<{ [date: string]: string }>(() => { try { const saved = sessionStorage.getItem('missedDayInputs'); return saved ? JSON.parse(saved) : {}; } catch { return {}; } });
 ...
 // Rollover state
 const [sleepInput, setSleepInput] = useState<string>(() => sessionStorage.getItem('rollover_sleepInput') || "");
 const [sleepMinsInput, setSleepMinsInput] = useState<string>(() => sessionStorage.getItem('rollover_sleepMinsInput') || "");
 const [screenInput, setScreenInput] = useState<string>(() => sessionStorage.getItem('rollover_screenInput') || "");
 const [screenMinsInput, setScreenMinsInput] = useState<string>(() => sessionStorage.getItem('rollover_screenMinsInput') || "");
 const [rolloverStep, setRolloverStep] = useState<number>(1);

  useEffect(() => {
    const handleGlobalClick = (e: Event) => {
      const target = e.target as HTMLElement;
      const isInteractive = target.closest('button, a, [role="button"], input[type="checkbox"], input[type="radio"], .cursor-pointer, .hover\\:scale-105');
      if (isInteractive) {
        vibrate(HAPTIC_PATTERNS.TAP);
      }
    };
  ...
  useEffect(() => {
    ...
    const processHistory = async () => {
      ...
      for (let i = 0; i < updatedHistory.length; i++) {
        ...
        if ((!fb || !fb.includes("Diagnosis:")) && !processedHistoryRef.current.has(updatedHistory[i].date)) {
 ...
 playTone(440.00, 0.0, 3.0, "sine", 0.15); // A4
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if ("Notification" in window && Notification.permission === "granted") {
        const { xpGainedToday, pendingTasks, todos, notificationSettings } = stateRef.current;
 ...
 if (hour === 18 && xpGainedToday < 100 && notificationSettings.motivationalAlerts) {
 ...
 if (hour === 21 && totalRemaining > 0 && notificationSettings.motivationalAlerts) {
 ...
 const taskInterval = setInterval(() => {
      if ("Notification" in window && Notification.permission === "granted" && stateRef.current.notificationSettings.taskReminders) {
 ...
 startTimeObj.setHours(parseInt(hours, 10) || 0, parseInt(minutes, 10) || 0, 0, 0);
 }
 ...
 : null;
          const latestWithFeedback = history.slice().reverse().find(e => e.aiFeedback) || latestEntry;
 if (!latestEntry)
 return (
 <div className="dark:text-white text-slate-900">Loading data...</div>
 );
 ...
 +{t.xpReward} XP
                      ...
                      <div className="bg-cyan-900/10 border border-cyan-800/50 p-5 rounded-2xl mt-4">
                        <h3 className="flex items-center gap-2 dark:text-cyan-400 text-cyan-700 font-bold mb-3 uppercase text-sm tracking-wider">
                          <BrainCircuit className="w-5 h-5" /> AI Coach Assessment
                        </h3>
                        <div className="dark:text-slate-300 text-slate-900 text-sm leading-relaxed space-y-3">
                          {latestWithFeedback.aiFeedback ? (
                            ...
                            <div className="text-slate-500 italic">
                              <p>No recent performance data to analyze.</p>
                            </div>
                          ) : (
 ...
 You either missed or severely underperformed on the following training days.
 The exam doesn't care about your excuses, but this system does. State exactly why you failed to execute. Be brutally honest.
 </p>
 ...
 >
 {isActive && (
 <div className={`absolute inset-0 rounded-lg animate-pulse pointer-events-none ${isProfile ? "bg-purple-500/10" : "bg-cyan-500/10"}`} />
 )}
 ...
 <Icon className={`w-4 h-4 transition-all duration-300 ${isActive ? "dark:text-purple-400 text-purple-700 drop-shadow-md icon-glow-purple" : "dark:text-purple-300 dark:text-purple-400 text-purple-700 group-hover:text-purple-200 icon-glow-purple"}`} />
 </div>
 ) : (
 <motion.div
 ...
 <span className={`font-bold tracking-wider text-[13px] ${isProfile ? (isActive ? "text-purple-100" : "text-purple-200") : ""} ${tab.id === "settings" ? "hidden" : ""}`}>
 {tab.label}
 </span>
 {isProfile && (
 ...
 Lv{level}
              ...
              {tab.id === "settings" && (
                <div className="hidden md:block absolute top-full mt-2 left-1/2 -translate-x-1/2 dark:bg-slate-800 bg-white border dark:border-slate-700 border-slate-200 shadow-xl rounded-lg px-3 py-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none z-50 whitespace-nowrap transform translate-y-1 group-hover:translate-y-0">
                  <span className="text-xs font-bold dark:text-slate-300 text-slate-700">{tab.label}</span>
                </div>
              )}
 ...
 <div className="absolute inset-0 rounded-full border border-purple-400/50 animate-[spin_4s_linear_infinite]" />
 <div className="absolute inset-0 rounded-full border border-purple-500/30 animate-[spin_3s_linear_infinite_reverse]" />
 <Icon className={`w-4 h-4 transition-all duration-300 ${isActive ? "dark:text-purple-400 text-purple-700 drop-shadow-md icon-glow-purple" : "dark:text-purple-300 dark:text-purple-400 text-purple-700 icon-glow-purple"}`} />
 </div>
 ) : (
 <motion.div
 ...
 <span className={`font-bold tracking-widest text-[9px] ${isProfile ? (isActive ? "text-purple-100" : "text-purple-200") : ""} ${tab.id === "settings" ? "hidden" : ""}`}>
 {tab.label}
 </span>
 {isProfile && (
 ...
 {isActive && (
 <div className={`absolute -bottom-px left-1 right-1 h-0.5 shadow-md rounded-full ${isProfile ? "bg-purple-400 shadow-md" : "bg-cyan-400"}`} />
 )}
 </motion.button>
 );
 })}
 ...
 <div className="flex-1 overflow-visible relative">
              <AnimatePresence mode="wait" custom={direction}>
                <motion.div
                  ...
                  transition={{ duration: 0.2, type: "spring", stiffness: 400, damping: 40 }}
 ...
 Made with{" "}
 <span className="dark:text-red-400 text-red-700 animate-pulse inline-block">❤️</span>{" "}
 by Madhav
 </p>
 </footer>
 </main>
