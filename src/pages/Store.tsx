...
export function generateArcadeCode(duration: number): string {
  ...
  const getPart = () => {
    ...
    if (typeof window !== "undefined" && window.crypto && window.crypto.getRandomValues) {
      ...
      for (let i = 0; i < 4; i++) {
        part += chars.charAt(array[i] % chars.length);
      }
    } else {
      for (let i = 0; i < 4; i++) {
  ...
  return `XM${duration}-${getPart()}-${getPart()}-${getPart()}`;
}
 ...
 const [arcadeKeys, setArcadeKeys] = useState<ArcadeKey[]>(() => {
  ...
  const handlePurchaseArcadePass = (pass: typeof ARCADE_PASSES[0]) => {
    ...
    if (spendableXP >= pass.cost) {
      ...
      let selectedCode = '';
      do {
        selectedCode = generateArcadeCode(pass.duration);
