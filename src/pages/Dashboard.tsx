...
const UncontrolledInput = React.memo(function UncontrolledInput({ placeholder, type = "text", autoFocus = false, onSubmit, buttonText, className }: any) {
 ...
 className={className || "flex-1 dark:bg-black bg-white border dark:border-slate-700 border-slate-300 rounded-lg px-4 py-2 dark:text-white text-slate-900 focus:border-cyan-500 outline-none"}
 autoFocus={autoFocus}
 />
 <Button
 variant="default"
...
const CalendarCheckbox = ({ checked, onChange }: { checked: boolean, onChange: (c: boolean) => void }) => (
  <div onClick={() => onChange(!checked)} className={`flex items-center gap-3 p-3 rounded-xl border border-dashed transition-all cursor-pointer select-none group mt-2 ${checked ? 'dark:bg-cyan-950/30 bg-cyan-50 dark:border-cyan-500/50 border-cyan-400 shadow-sm shadow-cyan-500/10' : 'dark:bg-black/50 bg-slate-50 dark:border-slate-800 border-slate-300 dark:hover:bg-slate-900 hover:bg-slate-100'}`}>
    <div className={`w-5 h-5 flex items-center justify-center rounded transition-all ${checked ? 'bg-cyan-500 shadow-md shadow-cyan-500/20' : 'dark:bg-slate-800 bg-slate-200 group-hover:bg-slate-300 dark:group-hover:bg-slate-700'}`}>
    ...
    <div className="flex flex-col">
       ...
       <span className="text-[10px] dark:text-slate-500 text-slate-500">Automatically creates an event block for this task</span>
    </div>
  </div>
);
...
export default function Dashboard() {
 ...
 aura_flame:
 "shadow-md border-amber-500/50 ring-2 ring-amber-500/20",
 aura_emerald:
 "shadow-md border-emerald-500/50 ring-2 ring-emerald-500/20",
 aura_solar:
 "shadow-md border-rose-500/50 ring-2 ring-rose-500/20",
 aura_neon:
 "shadow-md border-cyan-400/50 ring-2 ring-cyan-400/20",
 };
 ...
 <AnimatePresence>
 {showLiveDay && <LiveDayOverlay onClose={() => setShowLiveDay(false)} />}
 </AnimatePresence>
 <AnimatePresence>
 {toastMessage && (
 <motion.div
 ...
 {xpGainedToday} /{" "}
 <span className="dark:text-amber-400 text-amber-700">{bossDayTargetXp}</span>{" "}
 XP
 </div>
 ...
 <Shield className={`w-4 h-4 ${getRankInfo(level).color} ${getGlowClass(getRankInfo(level).color)}`} />
 <span className="text-xs font-bold dark:text-white text-slate-900">LVL {level}</span>
 </div>
 </div>
 </div>
 ...
 {/* Insights & Tips */}
 <div className={`flex flex-col gap-3 md:gap-4 justify-between h-full ${!class11EndDate ? 'xl:col-span-5' : 'xl:col-span-2'}`}>
 ...
 <AnimatedNumber value={Math.max(0, totalXpGoal - xp)} /> XP
 remaining to God Tier
 </p>
 </div>
 </div>
 ...
 <motion.div animate={pulseStreak ? { scale: [1, 1.3, 1], color: ['#fb923c', '#ffffff', '#fb923c'] } : {}} transition={{ duration: 0.5 }} className="text-2xl md:text-5xl font-black dark:text-white text-slate-900 drop-shadow-md">
                  {streakDays}{" "}
 ...
 (xpGainedToday / Math.max(1, Math.round(dailyXpRequired))) * 100 >= 70 
 ? 'bg-gradient-to-r from-emerald-600 to-green-400 shadow-md' 
 : (xpGainedToday / Math.max(1, Math.round(dailyXpRequired))) * 100 >= 40 
 ...
 STUDY PLAN
         ...
         <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="border-blue-500/50 h-8 px-3 dark:bg-blue-950/40 bg-blue-50 dark:text-blue-400 text-blue-600 hover:dark:bg-blue-900/60 hover:bg-blue-100 hover:border-blue-400 shadow-[0_0_12px_rgba(59,130,246,0.5)] transition-all font-bold gap-1.5 group"
          >
            <Calendar className="w-4 h-4 icon-glow-blue" />
            Calendar
          </Button>
        </div>
 </div>
 ...
 onClick={() =>
 handleAddTask(50, "Question Practice", "Practice", undefined, addToCalendarOption, customDuration)
 }
 >
 <motion.div
 whileHover={{ scale: 1.2, rotate: -10 }}
 ...
 onClick={() => handleAddTask(50, "DPP", "DPP", undefined, addToCalendarOption, customDuration)}
 >
 <motion.div
 whileHover={{ scale: 1.2, rotate: 15 }}
 className="relative"
 ...
 onClick={() =>
 handleAddTask(200, "Chapter Test", "Chapter Test", undefined, addToCalendarOption, customDuration)
 }
 >
 <motion.div
 whileHover={{ scale: 1.2, rotate: -15 }}
 ...
 onClick={() =>
 handleAddTask(50, "Revision", "Revision", undefined, addToCalendarOption, customDuration)
 }
 >
 <motion.div
 whileHover={{ scale: 1.2, rotate: 90 }}
 ...
 onClick={() => handleAddTask(100, "PYQs", "Practice", undefined, addToCalendarOption, customDuration)}
 >
 <motion.div
 whileHover={{ scale: 1.2, rotate: 15 }}
 className="relative"
 ...
 <form onSubmit={(e) => {
   e.preventDefault();
   const val = (e.currentTarget.elements.namedItem('taskName') as HTMLInputElement).value;
   if ((val || '').trim()) {
     handleAddTask(50, (val || '').trim(), "Custom", undefined, addToCalendarOption, customDuration);
   }
   ...
   <div className="flex gap-3">
     ...
     />
     <Button type="submit" variant="default" className="bg-cyan-600 hover:bg-cyan-500 text-white">
       Add
     </Button>
   </div>
   <CalendarCheckbox checked={addToCalendarOption} onChange={setAddToCalendarOption} />
{false && <label>
     <input
       type="checkbox"
       checked={addToCalendarOption}
       onChange={(e) => setAddToCalendarOption(e.target.checked)}
 ...
 <form onSubmit={(e) => {
   e.preventDefault();
   const val = (e.currentTarget.elements.namedItem('lectureNum') as HTMLInputElement).value;
   const finalVal = (val || '').trim() || predictNextLecture(selectedSubject || '', selectedChapter || '', todos, history, syllabus);
   if (finalVal) {
     handleAddTask(100, "Lecture", "Lecture", finalVal, addToCalendarOption, customDuration);
   }
 }} className="flex flex-col gap-3">
   <div className="flex gap-3">
     <input
       ...
       type="number"
       placeholder="e.g. 4"
       value={(hasEditedLecture ? lectureNumberInput : predictNextLecture(selectedSubject || '', selectedChapter || '', todos, history, syllabus)) || ''}
...
{false && <label>
 ...
 <form onSubmit={(e) => {
   ...
   const val = (e.currentTarget.elements.namedItem('taskName') as HTMLInputElement).value;
   if ((val || '').trim()) {
     handleAddTask(50, (val || '').trim(), "Quick Custom", undefined, addToCalendarOption, customDuration);
   }
 }} className="flex flex-col gap-3">
   <div className="flex gap-3">
     <input
...
{false && <label>
 ...
 >
 <div className={`absolute -top-10 -right-10 w-24 h-24 rounded-full blur-[30px] pointer-events-none ${isMustDo ? "bg-yellow-500/15" : "bg-red-500/10"}`} />
 <div>
 ...
 *Equip custom prestige titles in the Profile tab after
 ...
 <span className="dark:text-cyan-400 text-cyan-700 font-bold">1.</span>
 <span>
 ...
 <li className="flex gap-2 text-xs">
 <span className="dark:text-cyan-400 text-cyan-700 font-bold">2.</span>
 <span>
 ...
 <li className="flex gap-2 text-xs">
 <span className="dark:text-cyan-400 text-cyan-700 font-bold">3.</span>
 <span>
 ...
 <li className="flex gap-2 text-xs">
 <span className="dark:text-cyan-400 text-cyan-700 font-bold">4.</span>
 <span>
 ...
 document.body,
       ...
       <AnimatePresence>
        {showCalendar && <StudyCalendar onClose={() => setShowCalendar(false)} dailyXpRequired={Math.round(dailyXpRequired)} onToggleTodo={toggleTodo} />}
      </AnimatePresence>
</div>
 );
}
