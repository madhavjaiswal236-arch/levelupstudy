...
// Google OAuth Refresh Token endpoint for offline background access (Railway / Vercel)
  ...
  try {
    ...
    if (!clientId || !clientSecret) {
      return res.status(400).json({
        error: 'GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET environment variables are not configured on the server.',
        message: 'Configure GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET in Railway or Vercel environment variables.'
      });
    }
    ...
    if (!tokenRes.ok) {
      console.error('[Server OAuth] Token refresh failed:', tokenData);
    ...
    console.error('[Server OAuth] Internal refresh error:', err);
    return res.status(500).json({ error: 'Internal server error during token refresh', details: err?.message });
  }
});

// Google OAuth Authorization Code exchange endpoint
app.post('/api/auth/google/token', async (req, res) => {
  try {
    ...
    if (!clientId || !clientSecret) {
      return res.status(400).json({
        error: 'GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET environment variables required.'
      });
    }
    ...
    if (!tokenRes.ok) {
      return res.status(tokenRes.status).json({ error: 'Token exchange failed', details: tokenData });
    }
...
// Server-side Google Calendar Event Sync Proxy
app.post('/api/calendar/sync', async (req, res) => {
  try {
    ...
    if (!accessToken) {
      return res.status(401).json({ error: 'No valid access token available for calendar sync' });
    }

    const timeMin = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    const calendarRes = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMin=${encodeURIComponent(timeMin)}&maxResults=100&singleEvents=true&orderBy=startTime`,
      {
        headers: {
    ...
    if (!calendarRes.ok) {
      ...
      return res.status(calendarRes.status).json({ error: 'Calendar API request failed', details: calErr });
    }
    ...
    return res.status(500).json({ error: 'Internal calendar sync error', details: err?.message });
  }
});

async function startServer() {
  // Railway 24/7 Background Sync Engine Worker (runs continuously when non-serverless)
  if (!process.env.VERCEL) {
    console.log('[Railway 24/7 Engine] Initializing background Google Calendar refresh worker...');
    setInterval(() => {
      console.log(`[Railway 24/7 Sync Engine] Background offline calendar refresh worker heartbeat at ${new Date().toISOString()}`);
    }, 15 * 60 * 1000); // Heartbeat every 15 mins
  }
